...
2. Save skeleton to Hive immediately (app works without Codex)
...
- `maxHR = 220 - age` is passed to Codex for HR zone guidance
...
**Codex integration** (`lib/services/claude_service.dart`): Uses `Codex-sonnet-4-6`. One API call per week (batch), not per workout. Strips markdown code fences before JSON parsing. 401 → auth error, 429 → exponential backoff up to 3 retries, any other failure → silently skip (plan remains usable without enrichment). Profile data (age, weight, height) passed for personalized coaching.
